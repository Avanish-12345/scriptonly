package Drivers;

import java.net.MalformedURLException;
import java.net.URI;
import java.net.URISyntaxException;
import java.net.URL;
import java.time.Duration;

import org.openqa.selenium.remote.DesiredCapabilities;
import org.testng.annotations.Test;

import io.appium.java_client.android.AndroidDriver;
import io.appium.java_client.android.connection.ConnectionState;
import io.appium.java_client.android.connection.ConnectionStateBuilder;

public class Drivertogglemethods 
{
	@Test
public void methods() throws MalformedURLException, URISyntaxException
{
		DesiredCapabilities dc=new DesiredCapabilities();
		dc.setCapability("platformName", "android");
		dc.setCapability("appium:automationName","uiAutomator2");
		dc.setCapability("appium:deviceName", "vivo 1902");
		dc.setCapability("appium:UDID","LNJV7DSCGAS8LFBU");
		dc.setCapability("appium:noReset", true);
		dc.setCapability("appium:ignoreHiddenApiPolicyError",true);
		dc.setCapability("appium:autoGrantPermisions",true);
		URL u=new URI("http://localhost:4723").toURL();
		AndroidDriver driver=new AndroidDriver(u,dc);
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(28));
		//driver.toggleAirplaneMode();
		//driver.toggleWifi();
		//driver.toggleLocationServices();
		//driver.toggleData();
		//ConnectionState connection=driver.setConnection(new ConnectionStateBuilder().withAirplaneModeEnabled().build());
		//ConnectionState connection=driver.setConnection(new ConnectionStateBuilder().withDataDisabled().build());
		//ConnectionState connection=driver.setConnection(new ConnectionStateBuilder().withWiFiEnabled().build());
		ConnectionState connection=driver.setConnection(new ConnectionStateBuilder().withWiFiDisabled().build());
		
}
}
